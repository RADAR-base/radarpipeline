...
class TestIntegration(unittest.TestCase):
    ...
    def test_run(self):
        ...
        expected_df = pd.read_csv(os.path.join(expected_output_path,"phone_battery_charging_duration.csv"))
        ...
        self.assertIsFile(path)
        # read the file and verify that the output is the same"
        expected_df = pd.read_csv(os.path.join(expected_output_path,"step_count_per_day.csv"))
        actual_df = pd.read_csv(path)
        assert_frame_equal(expected_df, actual_df)
        path.unlink()