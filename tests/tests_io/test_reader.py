...
class TestSparkCSVDataReader(PySparkTestCase):
    def setUp(self):
        mock_config = {
            "input": {
                "config": {
                    "source_path": "tests/resources/test_data/"},
                "data_format": "csv"}}
        data_list = ['android_phone_step_count']
        self.sparkcsvdatareader = SparkCSVDataReader(self.spark, mock_config,
    ...
    def test_read_data(self):
        mock_dataframe = self.preprocess_data(self.mock_df)
        spark_data = self.sparkcsvdatareader.read_data()
        self.assertEqual(set(spark_data.get_data()["test_participant"].get_data()
                             ["android_phone_step_count"].get_data().collect()),
                         set(mock_dataframe.collect()))
        self.assertIsInstance(spark_data, RadarData)
        self.assertIsInstance(spark_data.get_data()["test_participant"], RadarUserData)
        self.assertIsInstance(spark_data.get_data()["test_participant"].get_data()
