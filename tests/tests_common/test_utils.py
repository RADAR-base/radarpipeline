...
class TestReadYaml(unittest.TestCase):
    ...
    def test_read_correct_yaml(self):
        ...
        expected_config = {'project': {'project_name': 'mock_project', 'description': 'mock_description', 'version': 'mock_version'}, 'input_data': {'data_location': 'local', 'local_directory': 'mockdata/mockdata', 'data_format': 'csv'}, 'configurations': {'df_type': 'pandas'}, 'features': [{'location': 'https://github.com/RADAR-base-Analytics/mockfeatures', 'branch': 'main', 'feature_groups': ['MockFeatureGroup'], 'feature_names': [['all']]}], 'output_data': {'output_location': 'local', 'local_directory': 'output/mockdata', 'data_format': 'csv', 'compress': False}}
