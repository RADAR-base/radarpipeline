...
class RadarData(Data):
    ...
    def get_combined_data(self) -> dd.DataFrame:
        return dd.concat([self._data[key].get_combined_data() for key in self._data.keys()]).reset_index(drop=True)

    def get_combined_data_by_variable(self, variable: Union[str, List[str]]) -> List[dd.DataFrame]:
        ...
        for var in variable:
            ...
            combined_arr.append(dd.concat([file_data[key].get_data(
            ) for file_data in files_data for key in file_data.keys()]).reset_index())
        if is_only_one_var:
            return combined_arr[0]
        return combined_arr

    def get_combined_data_by_user_id(self, user_id: Union[str, List[str]]) -> List[dd.DataFrame]:
