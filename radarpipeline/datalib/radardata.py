...
class RadarData(Data):
    ...
    def get_combined_data_by_variable(self, variable: Union[str, List[str]]) -> List[pd.DataFrame]:
        ...
        combined_arr = []
        for var in variable:
            variable_data_list = list(filter(partial(is_not, None), [self._data[key].get_data_by_key(var) for key in self._data.keys()]))
            files_data = [data.get_data() for data in variable_data_list]
            combined_arr.append(pd.concat([file_data[key].get_data() for file_data in files_data for key in file_data.keys() ]).reset_index())
        if is_only_one_var:
