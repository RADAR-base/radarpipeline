...
class MedianHeartRate(Feature):
    def __init__(self):
        ...
        self.required_input_data = [
            "connect_fitbit_intraday_heart_rate", "connect_fitbit_sleep_stages"]
    ...
    def calculate(self, data):
        ...
        hr_data_median = hr_data.groupby(["key.userId", "value.date"])[
            'value.heartRate'].median().reset_index()
...
class SdHeartRate(Feature):
    ...
    def calculate(self, data):
        ...
        hr_data = data.get_combined_data_by_variable(
            "connect_fitbit_intraday_heart_rate")
        hr_data['value.date'] = hr_data['value.time'].dt.date
        hr_data_std = hr_data.groupby(["key.userId", "value.date"])[
            'value.heartRate'].std().reset_index()
